      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressListFilter" :class="{'check': checkedIndex == index}"
                    @click="checked(item.addressId, index)">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="confirmDel(item.addressId)">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path: '/orderConfirm', query : {addressId: this.addressId}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      setDefault(addressId) {
        ...
        axios.post("/api/users/setDefault", {addressId: addressId}).then((response) => {
          let res = response.data
          res.status = 0
          // console.log('suc')
          /*
      ...
      delAddress() {
        this.modalConfirm = false
        axios.post('/api/users/delAddress', {addressId: this.addressId }).then((response) => {
          let res = response.data
        })
        this.init()
      },
      checked(addressId, index) {
